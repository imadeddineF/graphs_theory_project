...
export default function EdgeDrawn({
  ...
  const translateTextDistance =
    weight === "" ? 0 : -4.5 * Math.round(Math.log10(Math.abs(weight) + 1) + 1);
  const bezierX =
    centerX +
    (edge.u < edge.v ? 1 : -1) *
      ((isRight ? 1 : -1) *
        Math.min(length / 4, 50) *
  ...
  const textPosX = isCurved
    ? 0.25 * position.x1 + 0.5 * bezierX + 0.25 * position.x2
    : centerX;
  const textPosY = isCurved
    ? 0.25 * position.y1 + 0.5 * bezierY + 0.25 * position.y2
    : centerY;
    ...
    >
      ...
      {isDirected && (
        <path
          ...
          transform={`rotate(${(isCurved ? angle : alfa) + 90} ${position.x2} ${
            position.y2
          }) translate(0 18)`}
